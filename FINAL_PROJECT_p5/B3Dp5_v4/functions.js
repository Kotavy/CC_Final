...
function serialEvent() {
  ...
  if (inString.length > 0) { // This is greater that 0 bytes
    //if (inString !== 'hello') {
      var sensors = split(inString, ',');            // split the string on the commas
      if (sensors.length > 1) { // if there are three elements
        locationY = map(sensors[0], -20000, 20000, height/2, height);   // element 0 is the locH
        locationX = map(sensors[1], 20000, -20000, 0, width); // element 1 is the locV
      }
    //}
  serial.write('x');
  }
