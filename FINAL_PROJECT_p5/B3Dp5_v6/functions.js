...
function serialEvent() {
  ...
  if (inString.length < 39) { // This is greater that 0 bytes
      ...
      if (sensors.length > 1) { // if there are three elements
        locationY = map(sensors[0], -12000, 12000, height/2, height);
        locationX = map(sensors[1], 12000, -12000, -width*0.5, width*1.5);
        serial.write('x');
      }
  }
  else {
...
function resetSketch() {
  ...
  for (var co = 0; co < columns; co++) { // Loop over each column
    for (var ro = 0; ro < rows; ro++) {  // Loop over each row of the current column
      bricks[co][ro] = new brick(co, ro, r(ro), g(ro), b(ro), round(random(0, 3))); // Initialize a brick object in the array, at that (x,y)/(col,row) position
  ...
  for (var o = 0; o < orbs.length; o++) {
    if (orbs[o].alive === true) {
      tempArray[tempCounter] = new ball(locationX, locationY-10, orbs[o].bd, orbs[o].bc, orbs[o].bxs, -abs(orbs[o].bys), orbs[o].alive);
      tempCounter++;
    }
  }
  orbs = tempArray;
  newBall = new ball(width/2, 5*height/8, 20, 255, 5, -9);
  orbs.push(newBall);
  orbCount++;
}
...
function startup() {
  if (score === 0) {
    ...
    textSize(24);
    textAlign(CENTER);
    text("Welcome to Breakout: IE", width/2, height/2 - 1*height/10);
    text("(Interactive Edition)", width/2, height/2);
    text("SPACE to play.", width/2, height/2 + 1*height/5)
    if (start) {
      start = false;
      serial.write('c');
      gameActive = true;
...
function gameWon() {
  if (score == columns*rows) {
    ...
    text("Current Score: " + highscore + " points.", width/2, height/2);
    text("SPACE to level up.", width/2, height/2 + 1*height/5)
    serial.write('c');
    if (start) {
      start = false;
      serial.write('c');
...
function gameOver() {
  if (orbCount <= 0) {
    ...
    text("You Scored: " + highscore + " points.", width/2, height/2);
    text("SPACE to try again.", width/2, height/2 + 1*height/5);
    if (start) {
      highscore = 0;
      start = false;
      serial.write('c');
...
// Constructor
function ball(bx, by, bd, bc, bxs, bys, alive) {
  this.bx = bx;
  this.by = by;
  this.bd = bd;
  this.bc = bc;
  ...
  this.outOfBounds = function() {
    if (this.by - this.bd*2 > height) { // With padding
          this.bys = -(this.bys);

    // Comment out these two lines for debugging, the game will never end.
      orbCount--;
      this.alive = false;
    }
  }
  ...
  // Collision detection and handling for hand
...
function brick(x, y, r, g, b, s) {
  ...
  this.destroy = function() {
    if (this.special == 3) {
      newBall = new ball((this.brx + brWidth/2), (this.bry + brHeight/2), 20, gold, random(3, 6), random(-3, -9), true);
      orbs.push(newBall);
      orbCount++;
      //console.log(orbs);
    }
